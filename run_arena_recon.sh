#!/usr/bin/env bash
# Usage: ./run_arena_recon.sh <input_dir> <output_dir> [option]
# Runs Arena on every .pdb file in input_dir and writes <name>_AArecon.pdb to output_dir.
...
if [[ $# -lt 2 ]]; then
    ...
    echo "  option: Arena option (default 5)"
    exit 1
fi
...
for pdb in "${pdbs[@]}"; do
    ...
    out="$OUTPUT_DIR/${name}_AArecon.pdb"
